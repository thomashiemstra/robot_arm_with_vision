...
void IK::inverseKinematics(double x,double y,double z,double t[3][3],double angles[7], int flip){ /*results are stored in angles array */
    ...
    angles[5] = (pi/2.0) - (2.0/3.0)*angles[5] ;
	angles[6] = (pi/2.0) + 0.5*angles[6];
}

void IK::inverseKinematicsTest(double x,double y,double z,double t[3][3],vector<double>& angles) /*results are stored in angles array */
{
    /* the method here follows the calculations from the book:
    Modelling and Control of Robot Manipulators, Lorenzo Sciavicco and Bruno Siciliano chapter 2 (specifically 2.12.2)*/
    double xc,yc,zc;
